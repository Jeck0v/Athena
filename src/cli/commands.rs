...
use crate::boilerplate::{
    ...
    generate_laravel_project, generate_symfony_project, generate_vanilla_project,
...
pub fn execute_command(command: Option<Commands>, verbose: bool) -> AthenaResult<()> {
    match command {
        ...
        Some(Commands::Init(init_cmd)) => execute_init(init_cmd, verbose),
...
fn execute_init(init_cmd: InitCommands, verbose: bool) -> AthenaResult<()> {
    match init_cmd {
        InitCommands::Fastapi {
            ...
            if verbose {
                println!("Initializing FastAPI project: {}", name);
            }

            // Determine database type
            let database = if with_postgresql {
                DatabaseType::PostgreSQL
            } else {
                DatabaseType::MongoDB // Default to MongoDB
            };
            ...
            let config = ProjectConfig {
                ...
                framework: None, // Not applicable for FastAPI
            };

            generate_fastapi_project(&config)?;
            Ok(())
        }
        ...
        InitCommands::Flask {
            ...
            if verbose {
                println!("Initializing Flask project with {}: {}", db_type, name);
            }
            ...
            let config = ProjectConfig {
                ...
                database,
                include_docker: !no_docker,
                framework: None, // Not applicable for Flask
            };

            generate_flask_project(&config)?;
            Ok(())
        }
        ...
        InitCommands::Go {
            ...
            if verbose {
                println!("Initializing Go project: {} with {}", name, framework);
            }
            ...
            // Convert framework
            let go_framework = match framework {
                crate::cli::args::GoFramework::Gin => GoFramework::Gin,
                crate::cli::args::GoFramework::Echo => GoFramework::Echo,
                crate::cli::args::GoFramework::Fiber => GoFramework::Fiber,
            };
        ...
        InitCommands::Vanilla {
            ...
            let db_type = if with_mysql { "MySQL" } else { "PostgreSQL" };
            if verbose {
                println!("Initializing PHP Vanilla project with {}: {}", db_type, name);
            }
            ...
            let config = ProjectConfig {
                ...
                database,
                include_docker: !no_docker,
                framework: None, // Not applicable for Vanilla
            };

            generate_vanilla_project(&config)?;
            Ok(())
        }
    }
}
